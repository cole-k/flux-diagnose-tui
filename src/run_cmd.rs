...
use git2::{Repository, Oid, ErrorCode, ReferenceType, FetchOptions, FetchPrune, RemoteCallbacks}; // Added FetchOptions, FetchPrune, RemoteCallbacks
use log::{warn, info, error}; // Added error level
...
impl fmt::Display for GitInformation {
     fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Commit: {}, Branch: {}, Remote: {}, Relative Path: {}",
...
fn find_remote_url_containing_commit(repo: &Repository, commit_oid: Oid) -> Result<Option<String>> {
    info!("Searching local remote refs for commit {}", &commit_oid.to_string()[..7]);
    for reference_result in repo.references_glob("refs/remotes/*")? {
        ...
        if let Some(remote_branch_tip_oid) = reference.target() {
            ...
            match repo.graph_descendant_of(remote_branch_tip_oid, commit_oid) {
                Ok(true) => {
                    if let Some(ref_name) = reference.name() {
                        ...
                        if parts.len() >= 3 && parts[0] == "refs" && parts[1] == "remotes" {
                            ...
                            info!("Commit {} found on remote '{}' via local ref '{}'", &commit_oid.to_string()[..7], remote_name, ref_name);
                            match repo.find_remote(remote_name) {
                                Ok(remote) => {
                                    if let Some(url) = remote.url() {
                                        return Ok(Some(url.to_string()));
                                    } else {
                                        warn!("Remote '{}' (from ref '{}') has no URL configured.", remote_name, ref_name);
                                    }
                                }
                                Err(e) => warn!("Found ref '{}' but failed to find remote config for '{}': {}", ref_name, remote_name, e),
                            }
                        } else {
                ...
                Err(e) => {
                    if e.code() != ErrorCode::NotFound { // NotFound can happen if OIDs become invalid, less critical
                      warn!("Error checking commit ancestry for ref {:?} against commit {}: {}", reference.name(), &commit_oid.to_string()[..7], e);
                    }
                }
            }
        }
    }
    info!("Commit {} not found on any existing local remote refs.", &commit_oid.to_string()[..7]);
...
fn fetch_all_remotes_prune(repo: &Repository) -> Result<()> {
        ...
        git2::Cred::default() // Try default system credential helpers
            .or_else(|e| {
             ...
             .or_else(|e| {
                ...
                Err(e) // Return error to signal fetch failure for this remote
            })
    });
    ...
    for remote_name_res in remotes.iter() {
        match remote_name_res {
            Some(remote_name) => {
                 ...
                 match repo.find_remote(remote_name) {
                    Ok(mut remote) => { // `fetch` needs `&mut self`
                        ...
                        match remote.fetch(refspecs, Some(&mut fetch_options), None) {
                             ...
                             Err(e) => {
                                let msg = format!("Failed to fetch remote '{}': {}", remote_name, e);
                    ...
                    Err(e) => {
                         let msg = format!("Failed to find remote configuration for '{}' listed in remotes: {}", remote_name, e);
                         warn!("{}", msg);
    ...
    if fetch_errors.is_empty() {
        ...
        bail!("Fetch completed with errors for some remotes:\n - {}", fetch_errors.join("\n - "));
    }
}


pub fn run_flux_in_dir(
    directory: &Path,
) -> Result<(Vec<CompilerMessage>, GitInformation)> {
    let canonical_directory = directory.canonicalize()
        .with_context(|| format!("Failed to canonicalize directory path: {}", directory.display()))?;

    // --- Run the main command ---
    info!("Running command: FLUX_FLAGS=\"-Fdebug-binder-output\" cargo flux in {}", canonical_directory.display());
    ...
    let stdout_handle = child.stdout.take()
        .ok_or_else(|| anyhow::anyhow!("Failed to get stdout handle for command: {}", command_desc))?;

    let reader = BufReader::new(stdout_handle);
    let mut stdout_lines = Vec::new();
    for line_result in reader.lines() {
        let line = line_result.with_context(|| format!("Failed to read line from command stdout: {}", command_desc))?;
        let json: serde_json::Value  = serde_json::from_str(&line)?;
        // Does it have a "reason" field and is that reason "compiler-message"
        if json.as_object().and_then(|obj| obj.get("reason").and_then(|reason| reason.as_str().map(|r| r == "compiler-message"))).unwrap_or(false) {
    ...
    info!("Collecting Git information for directory: {}", canonical_directory.display());
    let repo = Repository::discover(&canonical_directory)
        .with_context(|| format!("Failed to discover git repository containing '{}'", canonical_directory.display()))?;
    ...
    let repo_root = repo.workdir()
        .ok_or_else(|| anyhow::anyhow!("Repository at '{}' is bare, cannot determine relative path or reliably prune.", repo.path().display()))?;

    let rel_path_from_root = pathdiff::diff_paths(&canonical_directory, repo_root)
        .ok_or_else(|| anyhow::anyhow!(
            "Could not determine relative path from repo root ({}) to directory ({})",
            repo_root.display(), canonical_directory.display()
        ))?;

    let head_ref = repo.head().context("Failed to get HEAD reference")?;
    let commit_oid = head_ref.peel_to_commit()
    ...
    info!("Resolved Git info: Commit {}, Branch '{}', Relative Path '{}'",
          &commit_oid.to_string()[..7], branch, rel_path_from_root.display());


    // --- Find Remote URL based on Commit ---
    info!("Attempting to find remote URL for commit {}", &commit_oid.to_string()[..7]);
    let mut remote_url = find_remote_url_containing_commit(&repo, commit_oid)?;

    if remote_url.is_none() {
        info!("Commit {} not found on existing local remote refs. Attempting fetch...", &commit_oid.to_string()[..7]);
        ...
        match fetch_all_remotes_prune(&repo) {
            Ok(_) => {
                ...
                info!("Fetch completed. Re-checking for remote containing commit {}", &commit_oid.to_string()[..7]);
                ...
                if remote_url.is_some() {
                    info!("Found remote URL after fetch.");
                     ...
                     info!("Commit {} still not found on any remote refs after fetch.", &commit_oid.to_string()[..7]);
                }
            }
            Err(fetch_err) => {
                // Fetch failed more fundamentally (e.g., couldn't list remotes, or returned bail!)
                 error!("Failed to fetch repository updates: {:?}. Proceeding without remote URL.", fetch_err);
